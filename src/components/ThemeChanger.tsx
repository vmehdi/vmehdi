"use client";
import { Moon, PaintRoller, Sun } from "lucide-react";
import { useTheme } from "next-themes";
import { useEffect, useState } from "react";

const themes = [
  { name: "light", icon: <Moon size={28} /> },
  { name: "dark", icon: <Sun size={28} /> }
...
export function ThemeChanger() {
  ...
  const onChangeTheme = () => {
    ...
    setTheme(themes[findIndex === 1 ? 0 : findIndex + 1].name);
  };
  ...
  return (
    <div className="hover:text-primary dark:hover:text-primary ml-auto cursor-pointer" onClick={onChangeTheme}>
      {themeSelect.icon}
    </div>
  );
}
